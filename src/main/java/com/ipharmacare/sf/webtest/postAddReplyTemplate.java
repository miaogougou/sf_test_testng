...
public class postAddReplyTemplate {	
     ...
     * @throws
     */
    public String getHttpRespone(String replyTemplate) throws Exception {
    	BasicCookieStore cookieStore = RequestCookiestore.getRequestCookies();
        CloseableHttpClient httpclient = HttpClients.custom()
                .setDefaultCookieStore(cookieStore)
                .build();
        url=GetRequestUrl.getRequestUrl("/api/v1/addReplyTemplate"); 
        
        try {
        	HttpUriRequest req = RequestBuilder.post()
                    .setUri(new URI(url))
                    .addParameter("replyTemplate", replyTemplate)
                    .build();
        	CloseableHttpResponse response = httpclient.execute(req);
            try {
            	HttpEntity entity = response.getEntity();
            	
                System.out.println("addReplyTemplate form post: " + response.getStatusLine());
