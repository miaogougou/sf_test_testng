...
public class getOptOperation {	
    ...
    public String getHttpRespone(String optRecipeId) throws Exception {
        ...
        CloseableHttpClient httpclient = HttpClients.custom()
                ...
                .build();
        url=GetRequestUrl.getRequestUrl("/api/v1/analysis/opt/getOperation"); 
        
        try {
        	HttpUriRequest req = RequestBuilder.get()
                    .setUri(new URI(url))
                    .addParameter("recipeId", optRecipeId)
                    .build();
        	CloseableHttpResponse response = httpclient.execute(req);
            try {
            	HttpEntity entity = response.getEntity();
            	
                System.out.println("getOperation form get: " + response.getStatusLine());
