...
public class getAuditPlanList {	
     ...
     * @throws
     */
    public String getHttpRespone(String auditplanid) throws IOException {
    	BasicCookieStore cookieStore = RequestCookiestore.getRequestCookies();
        CloseableHttpClient httpclient = HttpClients.custom()
                .setDefaultCookieStore(cookieStore)
                .build();
        if(null != auditplanid && ""!=auditplanid) {
        	url=GetRequestUrl.getRequestUrl("/api/v1/auditPlan/"+auditplanid);
        } else {
        	url=GetRequestUrl.getRequestUrl("/api/v1/auditPlanList");
        }
        try {
