...
public class getAuditRefuse {	
     ...
     * @throws
     */
    public String getHttpRespone(String id,String auditType) throws Exception {
    	BasicCookieStore cookieStore = RequestCookiestore.getRequestCookies();
        CloseableHttpClient httpclient = HttpClients.custom()
                .setDefaultCookieStore(cookieStore)
                .build();
        url=GetRequestUrl.getRequestUrl("/api/v1/auditRefuse"); 
        
        try {
        	HttpUriRequest req = RequestBuilder.get()
                    .setUri(new URI(url))
                    .addParameter("id", id)
                    .addParameter("auditType", auditType)
                    .build();
        	CloseableHttpResponse response = httpclient.execute(req);
            try {
            	HttpEntity entity = response.getEntity();
            	
                System.out.println("auditRefuse form get: " + response.getStatusLine());
