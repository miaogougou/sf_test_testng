...
public class getOptRecipeInfoallTest {
    
    public String Code=null,Data=null,OptRecipeInfoall=null,Result=null,optRecipeId=null;
    getOptRecipeInfoall req=new getOptRecipeInfoall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeInfoall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeInfoall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeInfoall,statusInfo.getMessage());
        }
    }
}
