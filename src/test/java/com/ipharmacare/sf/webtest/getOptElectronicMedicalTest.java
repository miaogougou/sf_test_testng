...
public class getOptElectronicMedicalTest {
    
    public String Code=null,Data=null,OptElectronicMedical=null,Result=null,optRecipeId=null;
    getOptElectronicMedical req=new getOptElectronicMedical();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeElectronicMedical_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptElectronicMedical=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptElectronicMedical,statusInfo.getMessage());
        }
    }
}
