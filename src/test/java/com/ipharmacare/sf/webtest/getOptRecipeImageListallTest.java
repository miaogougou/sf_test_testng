...
public class getOptRecipeImageListallTest {
    
    public String Code=null,Data=null,OptRecipeImageListall=null,Result=null,optRecipeId=null;
    getOptRecipeImageListall req=new getOptRecipeImageListall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeImageListall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeImageListall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeImageListall,statusInfo.getMessage());
        }
    }
}
