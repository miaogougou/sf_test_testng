...
public class getOptEngineMsgListTest {
    
    public String Code=null,Data=null,OptEngineMsgList=null,Result=null,optRecipeId=null;
    getOptEngineMsgList req=new getOptEngineMsgList();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptEngineMsgList_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptEngineMsgList=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptEngineMsgList,statusInfo.getMessage());
        }
    }
}
