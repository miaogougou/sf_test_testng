...
public class getOptEngineMsgListallTest {
    
    public String Code=null,Data=null,OptEngineMsgListall=null,Result=null,optRecipeId=null;
    getOptEngineMsgListall req=new getOptEngineMsgListall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptEngineMsgListall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptEngineMsgListall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptEngineMsgListall,statusInfo.getMessage());
        }
    }
}
