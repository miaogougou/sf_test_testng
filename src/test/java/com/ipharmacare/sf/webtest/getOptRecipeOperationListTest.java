...
public class getOptRecipeOperationListTest {
    
    public String Code=null,Data=null,OptRecipeOperationList=null,Result=null,optRecipeId=null;
    getOptRecipeOperationList req=new getOptRecipeOperationList();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeOperationList_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeOperationList=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeOperationList,statusInfo.getMessage());
        }
    }
}
