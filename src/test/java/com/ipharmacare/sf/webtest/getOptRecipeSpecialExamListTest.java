...
public class getOptRecipeSpecialExamListTest {
    
    public String Code=null,Data=null,OptRecipeSpecialExamList=null,Result=null,optRecipeId=null;
    getOptRecipeSpecialExamList req=new getOptRecipeSpecialExamList();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeSpecialExamList_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeSpecialExamList=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeSpecialExamList,statusInfo.getMessage());
        }
    }
}
