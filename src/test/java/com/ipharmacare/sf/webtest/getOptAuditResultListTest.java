...
public class getOptAuditResultListTest {
    
    public String Code=null,Data=null,OptAuditResultList=null,Result=null,optRecipeId=null;
    getOptAuditResultList req=new getOptAuditResultList();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptAuditResultList_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptAuditResultList=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptAuditResultList,statusInfo.getMessage());
        }
    }
}
