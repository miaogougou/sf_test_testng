...
public class getOptAuditResultListallTest {
    
    public String Code=null,Data=null,OptAuditResultListall=null,Result=null,optRecipeId=null;
    getOptAuditResultListall req=new getOptAuditResultListall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptAuditResultListall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptAuditResultListall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptAuditResultListall,statusInfo.getMessage());
        }
    }
}
