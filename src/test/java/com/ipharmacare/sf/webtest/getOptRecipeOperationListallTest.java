...
public class getOptRecipeOperationListallTest {
    
    public String Code=null,Data=null,OptRecipeOperationListall=null,Result=null,optRecipeId=null;
    getOptRecipeOperationListall req=new getOptRecipeOperationListall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeOperationListall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeOperationListall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeOperationListall,statusInfo.getMessage());
        }
    }
}
