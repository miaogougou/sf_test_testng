...
public class getOptRecipeAllergyListallTest {
    
    public String Code=null,Data=null,OptRecipeAllergyListall=null,Result=null,optRecipeId=null;
    getOptRecipeAllergyListall req=new getOptRecipeAllergyListall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeAllergyListall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeAllergyListall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeAllergyListall,statusInfo.getMessage());
        }
    }
}
