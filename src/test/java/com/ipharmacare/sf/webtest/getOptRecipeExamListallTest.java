...
public class getOptRecipeExamListallTest {
    
    public String Code=null,Data=null,OptRecipeExamListall=null,Result=null,optRecipeId=null;
    getOptRecipeExamListall req=new getOptRecipeExamListall();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeExamListall_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeExamListall=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeExamListall,statusInfo.getMessage());
        }
    }
}
