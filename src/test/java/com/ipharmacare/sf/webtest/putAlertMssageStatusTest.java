...
public class putAlertMssageStatusTest {
    
    public String Code=null,Data=null,Msg=null,Result=null;
    putAlertMssageStatus req=new putAlertMssageStatus();
    
    
    @Parameters({"messageId","status"})
	@Test(groups = { "BaseCase1"})
    public void putAlertMssageStatus_Succ(String messageId,String status) throws Exception{
        resultCheck(messageId,status,StatusInfo.REQUEST_SUCCESS); 
    }
     
    public void resultCheck(String messageId,String status,StatusInfo statusInfo ) throws Exception{
        Result=req.getHttpRespone(messageId,status);  
