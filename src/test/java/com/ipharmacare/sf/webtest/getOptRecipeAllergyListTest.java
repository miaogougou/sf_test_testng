...
public class getOptRecipeAllergyListTest {
    
    public String Code=null,Data=null,OptRecipeAllergyList=null,Result=null,optRecipeId=null;
    getOptRecipeAllergyList req=new getOptRecipeAllergyList();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeAllergyList_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeAllergyList=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeAllergyList,statusInfo.getMessage());
        }
    }
}
