...
public class getOptRecipeImageListTest {
    
    public String Code=null,Data=null,OptRecipeImageList=null,Result=null,optRecipeId=null;
    getOptRecipeImageList req=new getOptRecipeImageList();
    
    
    @Parameters({"optRecipeId"})
	@Test(groups = { "BaseCase1"})
    public void getOptRecipeImageList_Succ(String optRecipeId) throws Exception{
    ...
    public void resultCheck(String optRecipeId, StatusInfo statusInfo ) throws Exception{
        ...
        OptRecipeImageList=Common.getJsonValue(Result, "message");
        if(null != statusInfo ) {
        	Assert.assertEquals(Code,statusInfo.getCode()); 
            Assert.assertEquals(OptRecipeImageList,statusInfo.getMessage());
        }
    }
}
